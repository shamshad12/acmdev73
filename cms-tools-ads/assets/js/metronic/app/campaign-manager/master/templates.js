					...
					if (data.count) {
						for ( var i = 0; i < data.rows.length; i++) {
							...
							if (is_upload == '1')
								view = '<a href="../../ads/'
										+ page_confirm
										+ '" style="cursor:pointer" target="blank" title="Preview" class="btn green mini"><span class="icon-zoom-in"></a>';
                             else
                             view = '<a href="../../ads/page_preview?id='
										+ data.rows[i]['id']
										+ '" style="cursor:pointer" target="blank" title="Preview" class="btn green mini"><span class="icon-zoom-in"></a>';
							var zip = '';
							var media_type=2;
                            if(data.rows[i]['media_name'].trim()=='Wap Campaign')
                                media_type=1;
							if (is_upload == '1')
								zip = '<a  href="#" style="cursor:pointer" title="Download" class="btn green mini active" data-toggle="modal" onclick="downloadZip(\''
										+ splite[1]
										+ '\',\''+media_type+'\')"><span class="icon-download"></span></a>';
							...
							if (accessDelete == '1') {
								if (data.rows[i]['edit_type'] == 1
									...
									del = '<a  style="cursor:pointer;" class="btn red mini active" data-toggle="modal" onclick="deleteTemplates(\''
											+ data.rows[i]['id']
							...
							var upload_zip = '';
							if (is_upload == '1')
								upload_zip = '<a onclick="uploadNewTemplate(\''
										+ data.rows[i]['id']
										+ '\')" style="cursor:pointer" title="Update template" class="btn green mini"><span class="icon-upload"></span></a>';
							create_campaign = '';
							if (is_upload == '1')
								create_campaign = '<a href="campaign_manager/campaigns?template_id='
										+ data.rows[i]['id']
										+ '&country_id='
										+ data.rows[i]['country_id']
										+ '&media_id='
										+ data.rows[i]['id_campaign_media']
										+ '" style="cursor:pointer" title="Create Campaign" class="btn green mini"><span class="icon-plus-sign"></span></a>';
							var status;
							if (data.rows[i]['edit_type'] == 1
									&& (data.rows[i]['session_user_typeid'] == 2))
				...
				success : function(data) {
					...
					if (data.count) {
                       jQuery('#upload_file').removeAttr('disabled');
						...
						jQuery('#upload_id_media').val(data.id_campaign_media);
						jQuery('#upload_id_media').attr('disabled','disabled');
						jQuery('#upload_description').val(data.description);
						jQuery('#upload_status').val(data.status);
						var res = data.url.split("/");
						jQuery('#upload_folder').val(res[5]);
						jQuery('#btn-preview').css('display', 'block');

						jQuery('#btn_page_preview').css('display', 'block');
						jQuery('#btn_page_preview').attr('href',
								data.preview_page);
						// jQuery('#btn_page_preview').attr('href',
						// "http://202.153.129.88/ads/"+data.page_confirm);
...
function loadLockedTemplate()
{
	 ...
	 jQuery
			.ajax({
				url : domain + "campaign_manager/templates/loadLockedTemplate",
				dataType : "json",
				type : "POST",
					success : function(data) {
					...
					var result = '';
					if (data.count) {
						 html = '<button type="button" class="btn purple" onClick="unlokedTemplates()">Check-in</button><form method="post" action="#"><table class="table table-striped table-bordered" id="group-list"><thead><tr><th class="hidden-phone" width="90"><span><input style="margin:0" type="checkbox"  onClick="toggle(this)" /></span> All</th><th class="hidden-phone" width="90">Country</th><th class="hidden-phone">Name</th><th class="hidden-phone" width="80">Status</th><th class="hidden-phone" title="Created by" width="75">Created By</th><th class="hidden-phone" width="75" title="Updated by">Updated By</th></tr></thead><tbody id="data-table">';
						for ( var i = 0; i < data.rows.length; i++) {
							result += '<tr class="odd gradeX"><td><input type="checkbox" name="locked_list" value="'+data.rows[i]['id']+'"></td><td>'
									+ data.rows[i]['name_country']
									+ '</td><td>'
									+ data.rows[i]['name'] + '</td><td>'
									+ data.rows[i]['status']
						...
						html = '<table><thead><tr><th class="hidden-phone" width="500">No Locked Templates</th></tr>';
					}
					$("#table-append-locked").empty();
			$('#table-append-locked').append(
								html +result + '</tbody></table>');
				}
			});
	 

}
function unlokedTemplates()
{
	var allVals = [];
	$('input[name="locked_list"]:checked').each(function() {
       allVals.push($(this).val());
     });
	jQuery.ajax({
				url : domain + "campaign_manager/templates/unlokedTemplates",
				dataType : "json",
				type : "POST",
				data : {
					'allid' : allVals
				...
				success : function(data) {
								if (data.success) {
								loadTemplates(1);
								inactiveTab('locked_template_list');
			...
			.ajax({
				...
				success : function(data) {
					if (data.count) {
						...
						jQuery('#id_campaign_media').attr('disabled','disabled');
						jQuery('#description').val(data.description);
                        jQuery('#wap_css').val(data.css_file_data);
                        jQuery('#css_file_name').val(data.css_file_name);
						...
						if (data.is_uploaded == '1') {
							...
							jQuery('#btn_page_confirm').attr(
									'href',
									data.url + 'editor.php?data='
											+ data.decode_confirm);

							jQuery('#btn_page_status').css('display', 'block');
							jQuery('#btn_page_status').attr(
									'href',
									data.url + 'editor.php?data='
											+ data.decode_status);

							jQuery('#btn_page_footer').css('display', 'block');
							jQuery('#btn_page_footer').attr(
									'href',
									data.url + 'editor.php?data='
											+ data.decode_footer);

							jQuery('#btn_page_error').css('display', 'block');
							jQuery('#btn_page_error').attr(
									'href',
									data.url + 'editor.php?data='
											+ data.decode_error);

							jQuery('#btn_page_css').css('display', 'block');
							// jQuery('#btn_page_css').attr('href',
							// data.url+'editor.php?data='+data.decode_css);
							var res = data.url.split("/");
							jQuery('#btn_page_css').attr(
									'href',
									'http://54.169.14.129/ads/sceditor/editcss.php?data='
											+ data.decode_css + '&file='
											+ res[5]);
							...
							CKEDITOR.instances.page_confirm
									.setData(data.page_confirm);

							CKEDITOR.instances.page_status
									.setData(data.page_status);

							CKEDITOR.instances.page_error
									.setData(data.page_footer);//

							CKEDITOR.instances.page_error
									.setData(data.page_error);
						}
...
function save() {
	var id = jQuery('#id').val(), is_uploaded = jQuery('#is_uploaded').val(), name = jQuery(
			'#name').val(), id_country = jQuery('#id_country').val(), id_campaign_media = jQuery(
			'#id_campaign_media').val(),wap_css=jQuery('#wap_css').val(), page_confirm = CKEDITOR.instances.page_confirm
			.getData(), page_status = CKEDITOR.instances.page_status.getData(),file_name=jQuery('#css_file_name').val(),
	// page_footer = CKEDITOR.instances.page_footer.getData(),
	page_error = CKEDITOR.instances.page_error.getData(), status = jQuery(
			'#status').val(), description = jQuery('#description').val();
...
function save_static() {
	var id = jQuery('#static_id').val(), name = jQuery('#static_name').val(), id_country = jQuery(
			'#static_id_country').val(), id_campaign_media = jQuery(
			'#static_id_campaign_media').val(), id_template = jQuery(
			'#static_id_template').val(), static_description = jQuery(
			'#static_description').val(), static_conf_header_text = jQuery(
			'#static_conf_header_text').val(), static_conf_id_banner = jQuery(
			'#static_conf_id_banner').val(), static_conf_text = jQuery(
			'#static_conf_text').val(), static_conf_msisdn_prefix = jQuery(
			'#static_conf_msisdn_prefix').val(), static_conf_button_text = jQuery(
			'#static_conf_button_text').val(), static_conf_tc_description = jQuery(
			'#static_conf_tc_description').val(), static_conf_footer_text = jQuery(
			'#static_conf_footer_text').val(), static_thanks_header_text = jQuery(
			'#static_thanks_header_text').val(), static_thanks_id_banner = jQuery(
			'#static_thanks_id_banner').val(), static_thanks_text = jQuery(
			'#static_thanks_text').val(), static_thanks_sms_keyword = jQuery(
			'#static_thanks_sms_keyword').val(), static_thanks_sms_shortcode = jQuery(
			'#static_thanks_sms_shortcode').val(), static_thanks_button_text = jQuery(
			'#static_thanks_button_text').val(), static_thanks_tc_description = jQuery(
			'#static_thanks_tc_description').val(), static_thanks_footer_text = jQuery(
			'#static_thanks_footer_text').val(), status = jQuery(
			'#static_status').val();
...
function activateUploadField(media_id){
    if(media_id.trim()==1 || media_id.trim()==2)
    {
...
function save_upload() {
	var id = jQuery('#id').val(), name = jQuery('#upload_name').val(), id_country = jQuery(
			'#upload_id_country').val(), id_campaign_media = jQuery(
			'#upload_id_media').val(), id_template = jQuery(
			'#upload_id_template').val(), description = jQuery(
			'#upload_description').val(), page_confirm = jQuery(
			'#upload_page_confirm').val(), page_status = jQuery(
			'#upload_page_status').val(), page_pin = jQuery('#upload_page_pin')
			.val(), status = jQuery('#upload_status').val();
...
function clearFormStatic() {
	...
	jQuery('#static_description').val(''), jQuery('#static_conf_header_text')
			.val(''), jQuery('#static_conf_id_banner').val(''), jQuery(
			'#static_conf_text').val(''), jQuery('#static_conf_msisdn_prefix')
			.val(''), jQuery('#static_conf_button_text').val(''), jQuery(
			'#static_conf_tc_description').val(''), jQuery(
			'#static_conf_footer_text').val(''), jQuery(
			'#static_thanks_header_text').val(''), jQuery(
			'#static_thanks_id_banner').val(''), jQuery('#static_thanks_text')
			.val(''), jQuery('#static_thanks_sms_keyword').val(''), jQuery(
			'#static_thanks_sms_shortcode').val(''), jQuery(
			'#static_thanks_button_text').val(''), jQuery(
			'#static_thanks_tc_description').val(''), jQuery(
			'#static_thanks_footer_text').val(''), jQuery('#status').val('');
...
function clearFormUpload() {
	...
	jQuery('#upload_page_confirm').val(''), jQuery('#upload_page_status').val(
			''), jQuery('#status').val('');
...
function uploadFiles(event) {
	...
	data.append('upload_folder', upload_folder);
	var form = $('#formtemplates_upload');
	var error = $('.alert-error-upload-problem', form);
	var success = $('.alert-success-upload', form);
        var campaign_media_id=$('#upload_id_media').val();
        var new_url=domain + "campaign_manager/templates/uploadBackground?files";
        if(campaign_media_id.trim()==1)
        {
            new_url=domain + "campaign_manager/templates/uploadWapBackground?files";
        }

	jQuery('#upload_web_temp').html('Uploading Template...');
        jQuery('#upload_web_temp').attr('disabled','disabled');
...
function downloadZip(folder,media_type) {
    
	if (folder != "") {
		jQuery.ajax({
			url : domain + "campaign_manager/templates/downloadZip",
			dataType : "json",
			type : "POST",
			data : {
				'folder' : folder,
