...
//alert('hi');		
	var limit  = $('#limit').val(),
		search = $('#search').val(),
		html = '<table class="table table-striped table-bordered" id="group-list"><thead><tr><th class="hidden-phone">Language</th><th class="hidden-phone">Status</th><th class="hidden-phone">Action</th></tr></thead><tbody id="data-table">';
	
	jQuery.ajax({
			url:  domain+"campaign_manager/language/loadLanguage",
			dataType: "json",
			type: "POST",
			data: {
					'limit' : limit,
			...
			success: function(data) {
										...
										if(data.count){
											for(var i=0; i<data.rows.length; i++){
												...
												result  += '<tr class="odd gradeX"><td>'+data.rows[i]['language']+'</td><td>'+data.rows[i]['status']+'</td><td width="105">'+edit+' '+del+'</td></tr>';			
											}
										}
...
function edit(id){
	jQuery.ajax({
			url:  domain+"campaign_manager/language/getLanguageData",
			dataType: "json",
			type: "POST",
			data: {
					'id' : id
...
function save(){
	...
	jQuery.ajax({
			url:  domain+"campaign_manager/language/saveLanguage",
			dataType: "json",
			type: "POST",
			data: {
					'id' : id,
...
function deleteLanguage(){
	if(deleteID != ""){
		jQuery.ajax({
				url:  domain+"campaign_manager/language/deleteLanguage",
				dataType: "json",
				type: "POST",
				data: {
						'id' : deleteID
