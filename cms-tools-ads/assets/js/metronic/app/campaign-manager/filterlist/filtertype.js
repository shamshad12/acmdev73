function loadFiltertype(page){		 
	var limit  = $('#limit').val(), 
		search = $('#search').val(), 
		html = '<table class="table table-striped table-bordered" id="group-list"><thead><tr><th class="hidden-phone">List Name</th><th class="hidden-phone">Status</th><th class="hidden-phone">Action</th></tr></thead><tbody id="data-table">';
	     
	jQuery.ajax({
			url:  domain+"campaign_manager/filtertype/loadFiltertype",  
			dataType: "json", 
			type: "POST",
			data: {
					'limit' : limit,
                           ...
                           // alert(data);
										...
										if(data.count){
											for(var i=0; i<data.rows.length; i++){
												...
												result  += '<tr class="odd gradeX"><td>'+data.rows[i]['list_name']+'</td><td>'+data.rows[i]['status']+'</td><td width="100">'+edit+' '+del+'</td></tr>';			
											}
										}
...
function edit(id){
	jQuery.ajax({
			url:  domain+"campaign_manager/filtertype/getFiltertypeData", 
			dataType: "json",
			type: "POST",
			data: {
					'id' : id
			...
			success: function(data) {
										if(data.count){
											...
											jQuery('#list_name').val(data.list_name);  
...
function save(){
	...
	jQuery.ajax({
			url:  domain+"campaign_manager/filtertype/saveFiltertype",  
			dataType: "json", 
			type: "POST",
			data: {
...
function deletePartners(){
	if(deleteID != ""){
		jQuery.ajax({
				url:  domain+"campaign_manager/filtertype/deleteFiltertype",
				dataType: "json",
				type: "POST",
				data: {
						'id' : deleteID
