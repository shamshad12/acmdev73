...
function loadCampaigns(page, template_id, campaign_media, temp_country_id) {
	...
	jQuery
			.ajax({
				url : domain + "campaign_manager/campaigns/loadCampaigns",
				dataType : "json",
				type : "POST",
				data : {
					'limit' : limit,
					...
					if (data.count) {
						for ( var i = 0; i < data.rows.length; i++) {
							...
							if (data.rows[i]['template'])
								updateTemp = '<a href="campaign_manager/templates?template_id='
										+ data.rows[i]['template']
										+ '" style="cursor:pointer;" class="btn green mini" title="update template"><span class="icon-upload"></span></a>';
							var del = '';
							if (accessDelete == '1') {
...
function unlokedCampaign()
{
	...
	jQuery.ajax({
				url : domain + "campaign_manager/campaigns/unlokedCampaign",
				dataType : "json",
				type : "POST",
				data : {
					'allid' : allVals
				...
				success : function(data) {
								if (data.success) {
								loadCampaigns(1);
								inactiveTab('campaigns_loked_list');
...
function loadLockedCampaign()
{
	jQuery
			.ajax({
				url : domain + "campaign_manager/campaigns/loadLokedCampaigns",
				dataType : "json",
				type : "POST",
				success : function(data) 
				{
					...
					{
						html = '<table><thead><tr><th class="hidden-phone" width="500">No Locked Campaign found</th></tr>';
					}
					$("#table-append-lockedc").empty();
					$('#table-append-lockedc').append(
...
function edit(id) {
	jQuery.ajax({
		url : domain + "campaign_manager/campaigns/getCampaignsData",
		dataType : "json",
		type : "POST",
		data : {
			'id' : id
		...
		success : function(data) {
			...
			if (data.count) {
				...
				jQuery('#id_country').val(data.id_country);
					timezone(data.id_country);
				isTeraBilling(data.id_country,data.id_campaign_media);
				jQuery('#id_campaign_media').val(data.id_campaign_media);
				if (data.id_campaign_media == 2 || data.id_campaign_media == 6) { // web
					if(data.id_campaign_media == 2)
						$('#hide_show_confirmation_page').show();
					else
						$('#hide_show_confirmation_page').hide();
					$('#hide_show_confirmation_page').hide();
					$('#hide_show_is_virgin').hide();
					$('#hide_show_is_bypass').hide();
					$('#hide_show_start_time').hide();		//SAM			
					$('#hide_show_end_time').hide();		//SAM	
					...
					$('#hide_show_confirmation_page').show();
					$('#hide_show_is_virgin').show();
					$('#hide_show_is_bypass').show();
					$('#hide_show_start_time').show();		//SAM			
					$('#hide_show_end_time').show();		//SAM	
				...
				jQuery('#is_virgin').val(data.is_virgin);
				jQuery('#is_bypass').val(data.is_bypass);
				jQuery('#start_time').val(data.start_time); 	//SAM
				jQuery('#end_time').val(data.end_time);		//SAM
				...
				{
					$('#hide_show_start_time').parent().removeClass('hide')
					$('#hide_show_start_time').show();
					$('#hide_show_end_time').parent().removeClass('hide')
				...
				if (data.id_campaign_media == 2) {
					...
					var str = '<label class="control-label">Click to SMS<span class="required"> </span></label>'+	
						'<div class="controls">	'+	
						'	<div class="checkbox-list" id="domain_list">'+	
						'	<label><input type="checkbox" name="get_sms" id="get_sms" class="get_sms" value="1" '+checked+'></label>'+	
						'	</div>'+	
						'</div>';
					$('#hide_show_get_sms').html(str);
				}
			} else {
...
function loadUrl(id) {
	...
	jQuery.ajax({
		url : domain + "campaign_manager/campaigns/getUrlData",
		dataType : "json",
		type : "POST",
		data : {
			'id' : id
...
function loadUrlWithValues(id) {
	jQuery('#modal-body').html('');
	jQuery.ajax({
		url : domain + "campaign_manager/campaigns/getUrlDataWithValues",
		dataType : "json",
		type : "POST",
		data : {
			'id' : id
...
function save() {
	...
	content = jQuery('#content').val(), status = jQuery('#status').val(), expire_date_camp = jQuery(
			...
			.val(), is_virgin = jQuery('#is_virgin')
				.val(),is_bypass = jQuery('#is_bypass').val(), publish_start = jQuery('#publish_start').val(),start_time = jQuery('#start_time').val(),end_time = jQuery('#end_time').val(), publish_end = jQuery('#publish_end').val(),is_tera_billing = jQuery('#is_tera_billing').val(),get_sms = jQuery('#get_sms').val();
	...
	jQuery.ajax({
		url : domain + "campaign_manager/campaigns/saveCampaigns",
		dataType : "json",
		type : "POST",
		data : {
			'id' : id,
			...
			'thnk_msg' : thnk_msg,
			'alrt_msg' : alrt_msg,
			'start_time' : start_time, //SAM
			'end_time' : end_time, //SAM
			'is_tera_billing' : is_tera_billing, //RAM
			'get_sms' : get_sms //RAM
		},
		beforeSend : loading('portlet-form'),
		success : function(data) {
...
function deleteCampaigns(id) {
	...
	if (deleteID != "") {
		jQuery.ajax({
			url : domain + "campaign_manager/campaigns/deleteCampaigns",
			dataType : "json",
			type : "POST",
			data : {
				'id' : deleteID
...
function duplicateCampaigns() {
	if (duplicateID != "") {
		jQuery.ajax({
			url : domain + "campaign_manager/campaigns/duplicateCampaigns",
			dataType : "json",
			type : "POST",
			data : {
				'id' : duplicateID
...
function getTemplates(id_campaign_media, id_template) {
	if (id_campaign_media == 2 || id_campaign_media == 6) {
	...
	if (id_country != "") {
		jQuery.ajax({
			url : domain + "campaign_manager/templates/loadTemplatesSelect",
			dataType : "json",
			type : "POST",
			data : {
...
function changeStatus() {
	jQuery.ajax({
		url : domain + "campaign_manager/campaigns/Status",
		dataType : "json",
		type : "POST",
		data : {
			'id' : sid,
...
function isTeraBilling(country,media){	
	var id = $("#id").val();
	if(country ==''){
		country = $("#id_country").val();
	}
	if(media ==''){
		media = $("#id_campaign_media").val();
	}
	var str = "id : "+id+", Country : "+country+", media : "+media;
	//alert(str);
	$.post(domain + "campaign_manager/campaigns/isTeraBilling",{'media' : media,'country' : country,'id' : id},function(res) {
		 //alert(res);
		 $("#hide_show_is_tera_billing").html(res);
	});
}
...
$(document).ready(function() {
	...
	$('#id_campaign_media').on('change', sendSMS);
...
function timezone(id_country)
{
	...
	if (id_country != "") {
	jQuery.ajax({
			url:  domain+"campaign_manager/campaigns/timezone",
			dataType: "json",
			type: "POST",
			data: {
					'id_country' : id_country,
...
function getConfirm(id)
{
	if(id == 0){
		...
		$('#hide_show_start_time').hide();		
		$('#hide_show_end_time').hide();		
		var disabled = $('button[type=submit]').prop('disabled', false);
	}
	else
	{
		var start = $('#start_time').val();
		...
		if(start >= end)
		{
			var disabled = $('button[type=submit]').prop('disabled', true);	
		}
		$('#hide_show_start_time').parent().removeClass('hide')
		$('#hide_show_start_time').show();
		$('#hide_show_end_time').parent().removeClass('hide')		
...
function getStartdate(start_time)
{
	var end = $('#end_time').val();

	if((start_time!='' && start_time < end) || (start_time=='' && end==''))
  		var disabled = $('button[type=submit]').prop('disabled', false);
 	else
 		var disabled = $('button[type=submit]').prop('disabled', true); //SAM

}

function getEnddate(end_time)
{
	var start_time = $('#start_time').val();

	if((start_time!='' && end_time > start_time) || (start_time=='' && end_time==''))
  		var disabled = $('button[type=submit]').prop('disabled', false);
 	else 
 		var disabled = $('button[type=submit]').prop('disabled', true); //SAM

}
function sendSMS(media = null)
{
	if(media == ''){
		media = $('#id_campaign_media').val();
	}
	var str = '<label class="control-label">Click to SMS<span class="required">*</span></label>'+
				'<div class="controls">'+
				'	<select name="get_sms" id="get_sms" class="span6 m-wrap">'+
