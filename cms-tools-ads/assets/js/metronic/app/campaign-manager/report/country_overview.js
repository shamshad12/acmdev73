function loadcountry_overview(page){		
	...
	var result_ystrdy='';
	jQuery.ajax({
			url:  domain+"campaign_manager/country_overview/loadcountry_overview",
			dataType: "json",
			type: "POST",
			data: {
					'page'  : page,
											...
											if(data.today.length){
												for(var i=0; i<data.today.length; i++){
													if(i+1 == data.today.length)
														gt_style = ' gt_style';
													result_tdy  += '<tr class="odd gradeX'+gt_style+'"><td>'+data.today[i]['name_country']+'</td><td>'+data.today[i]['total_landing_views']+'</td><td>'+data.today[i]['total_web_entry']+'</td><td>'+data.today[i]['total_subscriber']+'</td><td>'+data.today[i]['subscriber_webentry']+'</td><td>'+data.today[i]['webentry_views']+'</td><td>'+data.today[i]['subscriber_views']+'</td></tr>';			
												}
											}
