					...
					if (data.count) {
						for ( var i = 0; i < data.rows.length; i++) {
							...
							if (data.rows[i]['template'])
								updateTemp = '<a href="partner_campaign_manager/templates?template_id='
...
function loadUrlWithValues(id) {
	...
	jQuery.ajax({
		url : domain
				+ "partner_campaign_manager/campaigns/getUrlDataWithValues",
		dataType : "json",
		type : "POST",
		data : {
			'id' : id
...
function deleteCampaigns(id) {
	...
	if (deleteID != "") {
		jQuery
				.ajax({
					url : domain
							+ "partner_campaign_manager/campaigns/deleteCampaigns",
					dataType : "json",
					type : "POST",
					data : {
						'id' : deleteID
...
function duplicateCampaigns() {
	if (duplicateID != "") {
		jQuery.ajax({
			url : domain
					+ "partner_campaign_manager/campaigns/duplicateCampaigns",
			dataType : "json",
			type : "POST",
			data : {
				'id' : duplicateID
...
function getTemplates(id_campaign_media, id_template) {
	...
	if (id_country != "") {
		jQuery.ajax({
			url : domain
					+ "partner_campaign_manager/templates/loadTemplatesSelect",
			dataType : "json",
			type : "POST",
			data : {
						...
						if (data.count) {
							...
							for ( var i = 0; i < data.rows.length; i++) {
								...
								result += "<div class='"
										...
										+ "><input type='checkbox' id='partner-service' value='"
				...
				success : function(data) {
					...
					if (data.count) {
						...
						for ( var i = 0; i < data.rows.length; i++) {
							...
							result += "<div class='"
									+ apcls
									+ "'><label class='span6 m-wrap'><input type='checkbox' id='ads-publisher' value='"
									+ data.rows[i]['id']
									+ "' "
									+ checked
									+ "> "
						...
						if (data.count) {
							for ( var i = 0; i < data.rows.length; i++) {
								...
								if (domain_list[data.rows[i]['id']])
									checked = "checked";
								result += "<label><input type='checkbox' id='domain-list' value='"
										+ data.rows[i]['id']
										+ "' "
										+ checked
										+ "> [ "
