					...
					if (data.count) {
						...
						if (data.is_uploaded == '1') {
							...
							jQuery('#btn_page_css').attr(
									'href',
									'http://202.153.129.88/ads/sceditor/editcss.php?data='
...
function save() {
	var id = jQuery('#id').val(), is_uploaded = jQuery('#is_uploaded').val(), name = jQuery(
			...
			'#id_campaign_media').val(), page_confirm = CKEDITOR.instances.page_confirm
			.getData(), page_status = CKEDITOR.instances.page_status.getData(),
...
function save_upload() {
	var id = jQuery('#id').val(), name = jQuery('#upload_name').val(), id_country = jQuery(
			...
			'#upload_id_campaign_media').val(), id_template = jQuery(
...
function uploadFiles(event) {
	...
	$.ajax({
		url : domain
				+ "partner_campaign_manager/templates/uploadBackground?files",
		type : 'POST',
		data : data,
		cache : false,
		dataType : 'json',
