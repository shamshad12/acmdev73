				...
				success : function(data) {
					...
					if (data.count) {
						for ( var i = 0; i < data.rows.length; i++) {
							...
							create_campaign = '';
							if (is_upload == '1')
								create_campaign = '<a href="partner_campaign_manager/campaigns?template_id='
										+ data.rows[i]['id']
										+ '&media_id='
										+ data.rows[i]['id_campaign_media']
										+ '&country_id='
										+ data.rows[i]['id_country']
...
function uploadNewTemplate(id) {
	...
	jQuery
			.ajax({
				url : domain
						+ "partner_campaign_manager/templates/getTemplatesData",
				dataType : "json",
				type : "POST",
				data : {
					'id' : id,
			...
			.ajax({
				...
				success : function(data) {
					...
					if (data.count) {
						...
						jQuery('#id_campaign_media')
								.val(data.id_campaign_media);
...
//console.log(id_campaign_media+'kkkkkk');
	var form = $('#formtemplates');
	var error = $('.alert-error-save', form);
	var success = $('.alert-success-save', form);

	jQuery.ajax({
		url : domain + "partner_campaign_manager/templates/saveTemplates",
		dataType : "json",
		type : "POST",
		data : {
			'id' : id,
		...
		success : function(data) {
			...
			if (data.success) {
				...
				$('.alert-error-upload').hide();
				$('.alert-success-upload').hide();
				$('.alert-error').hide();
				$('.alert-success').hide();
...
function save_static() {
	...
	jQuery
			.ajax({
				url : domain
						+ "partner_campaign_manager/templates/saveTemplatesStatic",
				dataType : "json",
				type : "POST",
				data : {
					'id' : id,
...
console.log(id_campaign_media+'kkkkkk');
	...
	jQuery
			.ajax({
				url : domain
						+ "partner_campaign_manager/templates/saveTemplatesUpload",
				dataType : "json",
				type : "POST",
				data : {
					'id' : id,
...
function deleteTemplates(id) {
	...
	if (deleteID != "") {
		jQuery
				.ajax({
					url : domain
							+ "partner_campaign_manager/templates/deleteTemplates",
					dataType : "json",
					type : "POST",
					data : {
						'id' : deleteID
...
function changeStatus() {
	...
	jQuery.ajax({
		url : domain + "partner_campaign_manager/templates/Status",
		dataType : "json",
		type : "POST",
		data : {
			'id' : sid,
...
function uploadFiles(event) {
	...
	var success = $('.alert-success-upload', form);
        ...
        {
            new_url=domain + "partner_campaign_manager/templates/uploadWapBackground?files";
        }
...
function downloadZip(folder,media_type) {
	...
	if (folder != "") {
		jQuery.ajax({
			url : domain + "partner_campaign_manager/templates/downloadZip",
			dataType : "json",
			type : "POST",
			data : {
				'folder' : folder,
