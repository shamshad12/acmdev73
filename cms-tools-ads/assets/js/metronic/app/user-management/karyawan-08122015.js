...
function loadKaryawan(page){	
	var limit  = $('#limit').val(),
		search = $('#search').val(),
		html = '<table class="table table-striped table-bordered" id="group-list"><thead><tr><th class="hidden-phone">ID</th><th class="hidden-phone">Nama Karyawan</th><th class="hidden-phone">Email</th><th class="hidden-phone">Telepon</th><th class="hidden-phone">Action</th></tr></thead><tbody id="data-table">';
	...
	jQuery.ajax({
			...
			success: function(data) {
										...
										for(var i=0; i<data.rows.length; i++){
											...
											result  += '<tr class="odd gradeX"><td>'+data.rows[i]['id']+'</td><td>'+data.rows[i]['nama_karyawan']+'</td><td>'+data.rows[i]['email']+'</td><td>'+data.rows[i]['telepon']+'</td><td>'+edit+' '+del+'</td></tr>';			
										}
