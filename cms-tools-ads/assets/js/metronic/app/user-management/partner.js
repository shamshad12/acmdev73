...
function loadPartner(page){	
	var limit  = $('#limit').val(),
		search = $('#search').val(),
		html = '<table class="table table-striped table-bordered" id="group-list"><thead><tr><th class="hidden-phone">Nama Partner</th><th class="hidden-phone">Email</th><th class="hidden-phone">Telepon</th><th class="hidden-phone">Action</th></tr></thead><tbody id="data-table">';
		
	jQuery.ajax({
			url:  domain+"user_management/partner/loadPartner",
			dataType: "json",
			type: "POST",
			data: {
					'limit' : limit,
			...
			success: function(data) {
										...
										for(var i=0; i<data.rows.length; i++){
											...
											result  += '<tr class="odd gradeX"><td>'+data.rows[i]['nama_partner']+'</td><td>'+data.rows[i]['email']+'</td><td>'+data.rows[i]['telepon']+'</td><td>'+edit+' '+del+'</td></tr>';			
										}
...
function edit(id){
	jQuery.ajax({
			url:  domain+"user_management/partner/getPartnerData",
			dataType: "json",
			type: "POST",
			data: {
					'id' : id
			...
			success: function(data) {
										if(data.count){
											...
											jQuery('#nama_partner').val(data.nama_partner);
...
function save(){
	var id = jQuery('#id').val(),
		...
		nama_partner = jQuery('#nama_partner').val(),
	...
	jQuery.ajax({
			url:  domain+"user_management/partner/savePartner",
			dataType: "json",
			type: "POST",
			data: {
					'id' : id,
...
function deletePartner(){
	if(deleteID != ""){
		jQuery.ajax({
				url:  domain+"user_management/partner/deletePartner",
				dataType: "json",
				type: "POST",
				data: {
						'id' : deleteID
...
function uploadFiles(event)
{
	...
	$.ajax({
		url: domain+"user_management/partner/uploadBackgroundAvatar?files",
		type: 'POST',
		data: data,
		cache: false,
		dataType: 'json',
