...
class karyawan extends Admin_Controller {
	...
	public function index() {		
		...
		$this->data['pageTemplate']		= "user-management/view_karyawan";
	...
	public function loadKaryawan(){
		$params = $_POST;
		$result = $this->model_karyawan->loadKaryawan($params);
		$result['pagination'] = $this->pagination($params['page'], $result['total'], $params['limit'], 'loadKaryawan');
	...
	public function saveKaryawan(){
		$params = $_POST;
		if(sizeof($params)){
			$result = $this->model_karyawan->saveKaryawan($params);
			echo json_encode($result);
		}
	}
	...
	public function deleteKaryawan(){
		if(sizeof($_POST)){
			$params = $_POST;
			$result = $this->model_karyawan->deleteKaryawan($params);
			echo json_encode($result);
		}
	}
	...
	public function getKaryawanData(){
		if(sizeof($_POST)){
			$params = $_POST;
			$result = $this->model_karyawan->getKaryawanData($params);
			echo json_encode($result);
		}
	}
	...
	public function duplicateKaryawan(){
		if(sizeof($_POST)){
			$params = $_POST;
			$result = $this->model_karyawan->duplicateKaryawan($params);
			echo json_encode($result);
		}
	}
}
