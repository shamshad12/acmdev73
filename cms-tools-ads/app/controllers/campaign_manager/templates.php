...
class templates extends Admin_Controller {
	...
	public function index() {
		...
		$this->data['pageTemplate']	= "campaign-manager/master/view_templates";
		$this->load->view('layout/main', $this->data);
	}

	public function loadLockedTemplate()
	{
		$params = $_POST;
		$result = $this->model_templates->loadLockedTemplate($params);
		echo json_encode($result);
	}
	public function unlokedTemplates()
	{
		$params = $_POST;
		$result = $this->model_templates->unlokedTemplates($params);
		echo json_encode($result);
	}
	...
	public function saveTemplates(){
		$params = $_POST;
		if(sizeof($params)){
                    if(isset($params['wap_css']) && trim($params['wap_css'])!='')
                        {
                        if(isset($params['file_name']) && trim($params['file_name'])!='')
                        {
                            $dir=$params['file_name'];
                        }else{
                            $dir=time().'.css';
                        }
                            $target_dir = "/var/www/html/ads/wap_template/css/".$dir;
                            $myfile = fopen($target_dir, "wa+") or die("Unable to open file!");
                            fwrite($myfile, $params['wap_css']);
                            $params['css_file_name']=$dir;
                            fclose($myfile);
	...
	public function getTemplatesData(){
		if(sizeof($_POST)){
			...
			if($result['rows'][0]['edit_type']==0)
			{
				...
				$result = $this->model_templates->getTemplatesData($params);
                                if(isset($result['css_file_name']) && trim($result['css_file_name'])!='')
                                {
                                    
                                    $target_dir = "/var/www/html/ads/wap_template/css/".$result['css_file_name'];
                                    chmod($target_dir, 0777);
                                    $myfile = file_get_contents($target_dir) ;
                                    $result['css_file_data']=$myfile;
        ...
        public function uploadWapBackground(){
		if(!isset($_GET['files']))
		{
		echo 
		json_encode(array('error' => 'Missing Parameter', 'status' => false));
		exit;
		}

		$dirUpload = $this->config->item('wap_template_upload');
		$dirAccess = $this->config->item('wap_template_access');
		$dirExtract = $this->config->item('wap_template_extract');
		...
		foreach($_FILES as $file)
		{
			...
			exec('chmod 0777 '.$dirUpload['campaigns']);
			exec('chmod 0777 '.$dirExtract['campaigns']);
			...
			if($result['status'] && isset($result['view']) && trim($result['view'])!=''){
				#Start to Extract

				#exec('mkdir -m 0777 '.$result['view']);
				
				if(!file_exists($result['view']))
				{
					exec('mkdir -m 0777 '.$result['view']);
					...
					exec('chmod 0777 '.$result['view']);
					//exec('mv -f '.$result['view'].' '.$result['temp_path']);
					exec('mv -f --backup=numbered '.$result['view'].' '.$result['temp_path']);
					//exec('mkdir -m 0777 '.$result['view']);
				}
				exec('unzip '.$result['path'].' -d '.$result['view']);
				exec('cp /var/www/html/ads/robots.txt '.$result['view']);
				//exec('chmod 0755 '.$result['view']);
				...
				if(count($dir)>=3){
					...
					if(!empty($baseFile) && !empty($result['view']))
					exec('mv -f '.$result['view'].$baseFile.'/* '.$result['view'].'/');
				}

				if((file_exists($result['view'].'/index.php')) || (file_exists($result['view'].'/index.html'))){
					...
					$result['pin'] = (!empty($pin))?$result['url'].$pin:''; 
					...
					$result['error'] 	= 'file must be index.php or index.html';
					$result['status'] 	= false;
				}
			}
			if(!empty($dirUpload['campaigns']))
			exec('chmod 0755 '.$dirUpload['campaigns']);
			if(!empty($dirExtract['campaigns']))
			exec('chmod 0755 '.$dirExtract['campaigns']);
			if(!empty($result['temp_path']))
			exec('chmod 0755 '.$result['temp_path']);
		}
		echo json_encode($result);
	}
	...
	public function downloadZip(){
		...
		{
			$cpath='/var/www/html/';
			if(!file_exists($cpath.'tempadszip/'.$param['folder'].'.tar '))
			{
				$curr = getcwd();
				//chdir('/var/www/html/ads/tpl/');
				if($_POST['media_type']==1)
					chdir('/var/www/html/ads/wap_tpl/');
                else
                    chdir('/var/www/html/ads/tpl/');
				exec('chmod 777 /var/www/html/tempadszip/');
				exec('tar cf ../../tempadszip/'.$param['folder'].'.tar '.$param['folder'].'/ --exclude="editor.php"');
				$result['url'] = 'http://'.$_SERVER['HTTP_HOST'].'/tempadszip/'.$param['folder'].'.tar';
				$result['count'] = True;
				#chdir($curr);
				exec('chmod 755 /var/www/html/tempadszip/');
			}
			else
			{
				$result['url'] = 'http://'.$_SERVER['HTTP_HOST'].'/tempadszip/'.$param['folder'].'.tar';
