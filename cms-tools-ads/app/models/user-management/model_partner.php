...
class model_partner extends MY_Model {
	...
	public function loadPartner($params){
		...
		$this->dbCms->where('k.type_account', 'Partner');
		...
		if($query->num_rows() != 0){
			...
			foreach($query->result_array() as $row) {
				...
				$result['rows'][$i]['nama_partner'] = $row['nama_karyawan'];
	...
	private function updatePartner($params){
		...
		$update = array(
						'store_id' => $this->dbCms->escape_str($params['store_id']),
						'nama_karyawan' => $this->dbCms->escape_str($params['nama_partner']),
	...
	private function insertPartner($params){
		...
		$input_time = strtotime(date('Y-m-d H:i:s'));
		$update = array(
						'type_account' => $this->dbCms->escape_str('Partner'),
						'store_id' => $this->dbCms->escape_str($params['store_id']),
						'nama_karyawan' => $this->dbCms->escape_str($params['nama_partner']),
						'tempat_lahir' => $this->dbCms->escape_str($params['tempat_lahir']),
						'tanggal_lahir' => $this->dbCms->escape_str($params['tanggal_lahir']),
		...
		if($this->dbCms->affected_rows()){
			...
			$partner_id = $this->dbCms->insert_id();
			
			#Insert into User table
			$update = array(
							'karyawan_id' => $this->dbCms->escape_str($partner_id),
							'username' => $this->dbCms->escape_str($params['username']),
							'userpass' => $this->dbCms->escape_str(md5($params['userpass'])),
							'tipe_user_id' => $this->dbCms->escape_str($params['tipe_user_id']),
	...
	public function getPartnerData($params){
		$this->dbCms->select('id, store_id, nama_karyawan, tempat_lahir, tanggal_lahir, jenis_kelamin, agama, alamat, email, telepon, status');
		$this->dbCms->from($this->tableKaryawan);
		$this->dbCms->where('id', $this->dbCms->escape_str($params['id']));
		$query = $this->dbCms->get();
		$result = array();
		...
		if($query->num_rows() != 0){
			...
			foreach($query->result_array() as $row) {
				...
				$result['nama_partner'] = $row['nama_karyawan'];
	...
	public function reportPartner($params = array()){
