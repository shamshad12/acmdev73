...
class model_templates extends MY_Model {
	...
	public function loadTemplates($params){
		...
		$this->dbAds->select('c.id, c.name, ct.name AS name_country, c.page_confirm, c.is_uploaded, c.status, c.page_status,c.edit_user,c.edit_type,c.description,c.entry_user,c.entry_time,c.update_user,c.update_time,c.id_campaign_media,ct.id AS country_id');
	...
	public function loadLockedTemplate($params)
	{
		$this->dbAds->select('c.id, c.name, ct.name AS name_country,c.status, c.entry_user,c.entry_time,c.update_user,c.update_time, c.edit_type');
		$this->dbAds->from('templates c');
		$this->dbAds->where('c.edit_type = 1');
		$this->dbAds->join('countries ct', 'c.id_country = ct.id');
		if(!empty($params['search']))
		$this->dbAds->where(" c.name LIKE '%".$params['search']."%'");
		$this->dbAds->order_by('c.id', 'DESC');
		$query = $this->dbAds->get();
		$i=0;
		if($query->num_rows() != 0){
	...
	public function unlokedTemplates($params)
	{
		if(count($params['allid'])>0)
		{	
//			$lokedin = implode('", "', $params['allid']);
			$update = array("edit_type" => 0);
	...
	public function loadTemplatesSelect($params = array()){
		$result = array();
		if($params['campaign_media']==6)
			$params['campaign_media']=2;
	...
	private function insertTemplates($params){
		...
		$update = array(
                        ...
                        'css_file_name'=> htmlspecialchars($params['css_file_name']),
						'status' 		=> $this->dbAds->escape_str($params['status']),
	...
	public function getTemplatesData($params){
		...
		$this->dbAds->where('id', $this->dbAds->escape_str($params['id']));
		$this->dbAds->update('templates', $update);
		$this->dbAds->select('id, name, id_country, id_campaign_media,css_file_name, description,page_confirm,status, page_status,page_footer, page_error,page_pin, is_uploaded');
		$this->dbAds->from('templates');
		$this->dbAds->where('id', $this->dbAds->escape_str($params['id']));
		$query = $this->dbAds->get();
		$result = array();
		...
		if($query->num_rows() != 0){
			...
			foreach($query->result_array() as $row) {
				...
				$result['preview_page'] = PATH_TEMPLATE.htmlspecialchars_decode($row['page_confirm']);
                $result['css_file_name']= $row['css_file_name'];
				if($row['is_uploaded']){
