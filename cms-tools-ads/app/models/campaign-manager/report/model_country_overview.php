...
class model_country_overview extends MY_Model {
	...
	public function getRecord($type,$result,$params)
	{
		...
		$this->dbAds->select('SUM(IF(status_campaign = 1, 1, 1)) AS total_landing_views, SUM(IF(status_campaign = 2, 1, 0)) AS total_web_entry, SUM(IF(transaction_id_billing IS NOT NULL, IF(delivery_status!="REJECTED",1,0), 0)) AS total_subscriber,name_country',false);
		$this->dbAds->group_by('id_country');
		$query = $this->dbAds->get($table);
		$i=0;
		if($query->num_rows() != 0){
			$result['count'] = true;
			foreach($query->result_array() as $row) {
				$result[$type][$i]['name_country']			= $row['name_country'];
			...
			$result[$type][$i]['name_country']		= "Grand Total";
	...
	public function loadcountry_overview($params){
		$result = array();
		$result['today'] = '';
		$result['yesterday'] = '';
		$result['count'] = false;
