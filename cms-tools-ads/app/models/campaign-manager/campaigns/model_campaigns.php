...
class model_campaigns extends MY_Model {
	...
	public function loadCampaigns($params){
		...
		$this->dbAds->select('c.id, c.name, c.content, c.status, c.use_confirmation,c.entry_user,c.edit_type,c.edit_user,c.entry_time,c.update_user,c.update_time,c.id_template,get_sms');
		...
		if($query->num_rows() != 0){
			...
			foreach($query->result_array() as $row) {
				...
				$result['rows'][$i]['get_sms'] 	= $row['get_sms'];
				//$result['rows'][$i]['id_language'] 	= $row['id_language'];
				if($row['language'])
	...
	{
		$this->dbAds->select('c.id, c.name, c.entry_user,c.edit_type,c.edit_user,c.entry_time,c.update_user,c.update_time');
		$this->dbAds->select('co.name AS country_name');
		$this->dbAds->select('cm.name AS media_name');
		$this->dbAds->select('cc.name AS categories_name');
		//$this->abAds->where('c.edit_type = "1"');
		$this->dbAds->where("c.edit_type ", 1);
	...
	public function unlokedCampaign($params)
	{
		if(count($params['allid'])>0)
		{	
			$update = array("edit_type" => 0, 'edit_user'=>'');
			if(is_array($params['allid']) && !empty($params['allid']))
			$this->dbAds->where_in('id',$params['allid']);
	...
	public function saveCampaigns($params){
		...
		$params['get_sms'] = 0;
		if(isset($params['get_sms']) && !empty($params['get_sms'])){
			$params['get_sms'] = 1;
		}
		$this->dbAds->select('id');
		$this->dbAds->from('campaigns');
	...
	private function updateCampaigns($params){
		...
		#'code_acm' 		=> $this->dbAds->escape_str($params['code_acm']),
						...
						'is_virgin' 	=> $this->dbAds->escape_str($params['is_virgin']),
						'is_bypass' 	=> $this->dbAds->escape_str($params['is_bypass']),
						'is_tera_billing' 	=> $this->dbAds->escape_str($params['is_tera_billing']),
						...
						'start_time' => $this->dbAds->escape_str($params['start_time']), //SAM
						'end_time' => $this->dbAds->escape_str($params['end_time']), //SAM
						'get_sms' => $this->dbAds->escape_str($params['get_sms'])	//RAM 
						);
						$this->dbAds->where('id', $this->dbAds->escape_str($params['id']));
						$this->dbAds->update('campaigns', $update);
	...
	private function insertCampaigns($params){
		...
		#'code_acm' 		=> $this->dbAds->escape_str($params['code_acm']),
						...
						'start_time' => $this->dbAds->escape_str($params['start_time']), //SAM
						'end_time' => $this->dbAds->escape_str($params['end_time']),	//SAM 
						'get_sms' => $this->dbAds->escape_str($params['get_sms'])	//SAM 
		);
	...
	public function getCampaignsData($params){
		...
		$this->dbAds->where('id', $this->dbAds->escape_str($params['id']));
		$this->dbAds->update('campaigns', $update);
		$this->dbAds->select('id, name, id_country, id_campaign_media, id_campaign_category, id_banner, id_template, id_language, code_acm, description,content,status, use_confirmation, is_virgin,is_bypass, expire_date_camp,publish_start,publish_end,start_time,end_time,get_sms');
		$this->dbAds->from('campaigns');
		$this->dbAds->where('id', $this->dbAds->escape_str($params['id']));
		$query = $this->dbAds->get();
		$result = array();
	...
	public function duplicateCampaigns($params){
		...
		$this->dbAds->select('id_country,id_campaign_media,id_campaign_category,id_banner,id_template,id_language,code_acm,
							content,name,description,use_confirmation,is_virgin,is_bypass,crossell,thanksmsg,alertmsg');
		$this->dbAds->from('campaigns');
		...
		if($query->num_rows() != 0){
			foreach($query->result_array() as $row) {
				...
				$result['thanksmsg'] = $row['thanksmsg'];
				$result['alertmsg'] = $row['alertmsg'];
			}
		}
		...
		$pars_ini = parse_ini_file('/var/www/html/ads/thanks_msg.ini', true);
		if(isset($result['thanksmsg']) && !empty($result['thanksmsg']))
		{
			$thnk_msg = str_replace("\\n", "", $result['thanksmsg']);
			if(isset($thnk_msg) && !empty($thnk_msg))
			{
				// unset($result['thanksmsg']);
				$pars_ini['thanks-msg'][$campaignId] = $thnk_msg;
			}
		}
		else
		...
		if(isset($result['alertmsg']) && !empty($result['alertmsg']))
		{
			$alrt_msg = str_replace("\\n", "", $result['alertmsg']);
			if(isset($alrt_msg) && !empty($alrt_msg))
			{
				// unset($result['alertmsg']);
				$pars_ini['alert-msg'][$campaignId] = $alrt_msg;
			}
		}
		else
	...
	function timezone($params)
	{
		...
		$this->dbAds->where('id', $params['id_country']);
		$query = $this->dbAds->get();
		if($query->num_rows()>0){
			$row = $query->result_array();
			$time = $row[0]['code'];
		...
		if(!empty($time))
		{
		$tmzn = DateTimeZone::listIdentifiers(DateTimeZone::PER_COUNTRY, strtoupper($time));
		date_default_timezone_set($tmzn[0]);
		}
		return date("F d, Y H:i:s", time());
	}
	public function isTeraBilling($params){//campaigns_media//countries
		$media_code_list = array('wap');
		$country_code_list = array('my','th');
		$result = array();
		//get campaigns info
		$is_tera_billing = 0;
		if(isset($params['id']) && !empty($params['id'])){
			$this->dbAds->select('is_tera_billing');
			$this->dbAds->from('campaigns');
			$this->dbAds->where('id', $this->dbAds->escape_str($params['id']));
			$query = $this->dbAds->get();
			$campaigns = $query->result_array();
			$is_tera_billing = $campaigns[0]['is_tera_billing'];
		}
		//media
		$this->dbAds->select('id,code,name');
		$this->dbAds->from('campaigns_media');
		$this->dbAds->where('id', $this->dbAds->escape_str($params['media']));
		$query = $this->dbAds->get();
		$media = $query->result_array();
		//country
		$this->dbAds->select('id,code,name');
		$this->dbAds->from('countries');
		$this->dbAds->where('id', $this->dbAds->escape_str($params['country']));
		$query = $this->dbAds->get();
		$country = $query->result_array();
		$str = $media[0]['code'].strtolower($country[0]['code']);
		 $res = 0;
		if(in_array($media[0]['code'],$media_code_list) && in_array(strtolower($country[0]['code']),$country_code_list)){
			$res = 1;
		}else{
			$res = 0;
		}
		$result['is_tera_billing'] = $is_tera_billing;
		$result['status'] = $res;
		return $result;
	}
}
