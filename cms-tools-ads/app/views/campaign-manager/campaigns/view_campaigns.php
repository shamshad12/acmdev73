				<div id="dashboard">
                  ...
                  <div class="tabbable tabbable-custom boxless">
                     <ul class="nav nav-tabs">
						...
						<li class="campaigns_loked_list "><a  onclick="loadLockedCampaign()" href="#campaigns_loked_list" data-toggle="tab">Locked Campaign List</a></li>
                     </ul>
                        ...
                        <br />  
                           <div class="portlet box blue portlet-list">
                              ...
                              <div class="portlet-body form">
                               <div class="btn-group">								
									<select class="m-wrap" id="search_user" name="search_user" style="width:100px;">
										<option value="">- User -</option>
										<?php for($i=0; $i<count($user_list['rows']); $i++){ ?>
										<option value="<?php echo $user_list['rows'][$i]['id'];?>"><?php echo $user_list['rows'][$i]['username'];?></option>
										<?php } ?>
									</select>
								 </div>
								  <div class="btn-group">								
						 ...
						 <!--Locked campaign List -->
						<div class="tab-pane" id="campaigns_loked_list">
							<br />
							<div class="portlet box blue portlet-list">
								<div class="portlet-title">
									<div class="caption">
										<i class="icon-reorder"></i>Locked Campaign List
									</div>
								</div>
								<div class="portlet-body form">
									
								<div id="table-append-lockedc"></div>
								</div>
							</div>
						</div>
			<!-- end of Locked campaign List -->
                              ...
                              <div class="portlet-body form">                              
                                 <form action="#" id="formcampaigns" class="form-horizontal">
									...
									<div class="control-group">
										...
										<div class="controls">
											<select name="id_country" id="id_country" class="span6 m-wrap" onchange="getPartners_Services(this.value, '', ''), getDomainsList(this.value, ''), getAds_Publishers('', '', this.value), timezone(this.value), isTeraBilling(this.value,'')">
									...
									<div class="control-group">
										...
										<div class="controls">
											<select name="id_campaign_media" id="id_campaign_media" class="span6 m-wrap" onchange="getTemplates(this.value, ''), getPartners_Services('', '', this.value), isTeraBilling('',this.value)">
...
<!-- end -->
									...
									<!-- SAM -->
									<div class="control-group hide"> 
										<div id="hide_show_start_time">
										<label class="control-label">Start Time<span class="required"></span></label>
										<div class="controls">	
											<select name="start_time" id="start_time" class="span3 m-wrap small" onchange="getStartdate(this.value)">
												<option value=''>--</option>
												<?php for($st=00; $st<24; $st++){ ?>
												<option value='<?php echo str_pad($st, 2, "0", STR_PAD_LEFT) ?>' ><?php echo str_pad($st, 2, "0", STR_PAD_LEFT) ?></option>
												<?php } ?>
											</select>
										</div>
										</div>
									...
									<div class="control-group hide">
										<div  id="hide_show_end_time">
										<label class="control-label">End Time<span class="required"></span></label>
										<div class="controls">
											<select name="end_time" id="end_time" class="span3 m-wrap small" onchange="getEnddate(this.value)">
												<option value=''>--</option>
												<?php for($et=00; $et<24; $et++){ ?>
												<option value='<?php echo str_pad($et, 2, "0", STR_PAD_LEFT) ?>' ><?php echo str_pad($et, 2, "0", STR_PAD_LEFT) ?></option>
												<?php } ?>
											</select>
										</div>
										</div>
									...
									<div class="control-group hide" id="hide_show_is_virgin">
										<label class="control-label">Is Virgin?<span class="required">*</span></label>
										<div class="controls">
										   <select name="is_virgin" id="is_virgin" class="span6 m-wrap">
											<option value="1">Yes</option>
											<option value="2">No</option>
										   </select>
										</div>
									</div>
									<div class="control-group hide" id="hide_show_is_bypass">
										<label class="control-label">Is Bypass?<span class="required">*</span></label>
										<div class="controls">
											<select name="is_bypass" id="is_bypass" class="span6 m-wrap">
												<option value="1">Yes</option>
												<option value="2">No</option>
											</select>
										</div>
									</div>
									<div class="control-group" id="hide_show_is_tera_billing">
										<input type="hidden" name="is_tera_billing" id="is_tera_billing" value="0">
									</div>
									<div class="control-group" id="hide_show_get_sms">
										<input type="hidden" name="get_sms" id="get_sms" value="0">
									</div>
									<div class="control-group">
    ...
    <script src="assets/js/metronic/app/campaign-manager/campaigns/campaigns.js?<?php echo date('H:i:s');?>" type="text/javascript"></script>
	<script type="text/javascript">
		...
		jQuery(document).ready(function() {
			...
			$('#id_campaign_media').change(function(){
				var str = '<label class="control-label">Click to SMS<span class="required"> </span></label>'+	
							'<div class="controls">	'+	
							'	<div class="checkbox-list" id="domain_list">'+	
							'	<label><input type="checkbox" name="get_sms" id="get_sms" class="get_sms" value="1"></label>'+	
							'	</div>'+	
							'</div>';				
				var media = $(this).val();
				if(media==2){
...
/*	 jQuery('#publish_end').datetimepicker({
	...
	$(document).on('change','#search_media',function(){
		alert($(this).val());
	});
    </script>
...
<div id="changestatus" class="modal hide fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
	...
	<div class="modal-header">
        ...
        <div class="modal-body">
	    <iframe name="alchemy_content" id="iframe_content" src="" style="zoom:0.60" frameborder="0" height="660" width="100%"></iframe>		
        </div>
        <div class="modal-footer">
            <button type="button" data-dismiss="modal" class="btn">Cancel</button>
