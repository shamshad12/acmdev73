...
{    
    ...
    public function newCommand($string)
    {
       
    $slices  = explode(' ', $string);
    $test_slice[0]=$slices[0];
    $test_slice[1]=$slices[1];
    unset($slices[0],$slices[1]);
    $slices[2]=implode(' ', $slices);
    $test_slice[2]=$slices[2];

    $request = $this->_encode_request($test_slice[0], array_slice($test_slice, 1));
